...
for i in range(runs):
    ...
    cost.extend(cost_over_time)
    cond_ent.extend(cond_entropy_over_time)
    sig_ent.extend(signal_entropy_over_time)
    run.extend(np.repeat(i+1, len(cost_over_time)))
...
df.to_csv(f'data/rep_over_time.csv', index=False)
