...
# ggsave(file = "figures/model/replication.png", height = 7)
```
...
```{r}
data_3d <- read_csv("data/3d_n=6_v=0.0007716049382716049_l=0.41.csv")
```
...
ent <- ggplot(avg_data_3d %>% filter(metric == "combined_entropy_over_time"), aes(x=time, y=m)) +
  geom_path() +
  geom_ribbon(aes(ymin = m - se, ymax = m + se), alpha = 0.3) +
  ylab(bquote(H[n](S[1]+S[2]))) + # "H(S1+S2)"
  xlab("Time (1 unit = 250 timesteps)") +
  theme_bw()
...
jsd <- ggplot(avg_data_3d %>% filter(metric == "jsd_over_time"), aes(x=time, y=m)) +
  geom_path() +
  geom_ribbon(aes(ymin = m - se, ymax = m + se), alpha = 0.3) +
  ylab(bquote(JSD(S[1],S[2]))) + # "JSD(S1,S2)"
  xlab("Time (1 unit = 250 timesteps)") +
  theme_bw()

ent + jsd

ggsave(file = "figures/model/3d_trends_horiz.png", width = 12)
```

# 4D figures

```{r}
data <- read_csv("data/4d_n=6_l=0.6_v=2.168374261397517e-05_t=2592.csv")
```
...
mi <- ggplot(avg_data %>% filter(metric == "ref_align_mi"), aes(x=time, y=m)) +
  geom_path() +
  geom_ribbon(aes(ymin = m - se, ymax = m + se), alpha = 0.3) +
  ylab(bquote(I(R[1],R[2]))) + # "MI(S1,S2)"
  xlab("Time (1 unit = 250 timesteps)") +
  theme_bw()
...
# ggsave(file = "figures/model/4d_trends_horiz.png", width = 12)
```
...
wider_data <- avg_data %>%
  pivot_wider(id_cols = c(time), names_from = metric, values_from = m) %>%
  select(combined_entropy_over_time, jsd_over_time, ref_align_mi)

print("X = entropy; Y = mi")
print(transfer_entropy(wider_data$combined_entropy_over_time, wider_data$ref_align_mi))

print("X = jsd; Y = mi")
print(transfer_entropy(wider_data$jsd_over_time, wider_data$ref_align_mi))

print("X = entropy; Y = jsd")
print(transfer_entropy(wider_data$combined_entropy_over_time, wider_data$jsd_over_time))
```
