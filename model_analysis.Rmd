...
ggplot(avg_data_2d, aes(x = time, y = m, linetype = metric)) +
  geom_path() +
  geom_ribbon(aes(ymin = m - se, ymax = m + se), alpha = 0.3, linetype = 0) +
  labs(x = "Time (1 unit = 250 timesteps)", y = "Value", linetype = "Measure") +
  theme_bw()
```
...
mi <- ggplot(avg_data %>% filter(metric == "ref_align_mi"), aes(x=time, y=m)) +
  geom_path() +
  geom_ribbon(aes(ymin = m - se, ymax = m + se), alpha = 0.3) +
  ylab(bquote(MI[ ](S[1],S[2]))) + # "MI(S1,S2)"
  theme_bw() +
  theme(axis.title.x = element_blank())
